// SpaceHub Installer
// Entry point script for SpaceHub installation and configuration
// Users paste this script into JS Injector instead of injector.js

(function() {
    'use strict';

    console.log('[SpaceHub Installer] Initializing...');

    const MODAL_ID = 'SpaceHubSourceModal';
    ...
    async function isAdmin() {
        try {
            ...
            console.warn('[SpaceHub Installer] Could not check admin status:', error);
        }
        return false;
    }
    ...
    async function findJavaScriptInjectorPlugin() {
        try {
            ...
            console.error('[SpaceHub Installer] Error finding JavaScript Injector plugin:', error);
            throw error;
        }
    }
    ...
    async function getJavaScriptInjectorConfig(pluginId) {
        try {
            ...
            console.error('[SpaceHub Installer] Error getting JS Injector config:', error);
            throw error;
        }
    }

    // Get SpaceHub config from JS Injector
    async function getSpaceHubConfig() {
        try {            
            if (window.SpaceHubConfig) {
                return window.SpaceHubConfig;
            }
            ...
            if (SpaceHubScript && SpaceHubScript.Script) {
                ...
                if (scriptMatch && scriptMatch[1]) {
                    try {
                        ...
                        console.error('[SpaceHub Installer] Error parsing config:', parseError);
                    }
                }
            }
        } catch (error) {
            console.warn('[SpaceHub Installer] Error getting config:', error);
        }
        return null;
    }
    ...
    async function loadDefaultConfig(rootUrl) {
        return new Promise((resolve, reject) => {
            try {
                const defaultConfigUrl = `${rootUrl}SpaceHub-default-config.js`;
    ...
    async function saveConfigToJavaScriptInjector(config) {
        try {
            ...
            const scriptContent = `// SpaceHub Configuration
// This file is automatically generated by SpaceHub Installer
// Do not edit manually unless you know what you're doing

window.SpaceHubConfig = ${JSON.stringify(config, null, 2)};`;
            ...
            console.error('[SpaceHub Installer] Error saving config:', error);
            throw error;
        }
    }
    ...
    function parseSpaceHubSource(url) {
        if (!url || url === '') {
            return { sourceType: 'github', version: LATEST_RELEASE_NAME };
        }

        const match = url.match(/cdn\.jsdelivr\.net\/gh\/ranaldsgift\/SpaceHub@([^\/]+)\//);
    ...
    async function fetchVersions() {
        ...
        try {
            ...
            console.warn('[SpaceHub Installer] Error fetching versions:', error);
            return [LATEST_RELEASE_NAME, DEVELOPMENT_NAME, EXPERIMENTAL_NAME];
        }
    }

    // Build SpaceHubRoot URL from selections
    function buildSpaceHubRootUrl(sourceType, source) {
    ...
    function showConfirmationModal(message, onConfirm, onCancel = null) {
        const CONFIRM_MODAL_ID = 'SpaceHubConfirmModal';
    ...
    function showAlertModal(title, message) {
        const ALERT_MODAL_ID = 'SpaceHubAlertModal';
    ...
    async function showSuccessModal(isInitialInstall, version) {
        const SUCCESS_MODAL_ID = 'SpaceHubSuccessModal';
        ...
        if (isInitialInstall) {
            ...
            content.innerHTML = `
                <p>SpaceHub configuration has been successfully updated. Please refresh your page for the changes to take effect!</p>
            `;
        }

        // Add discord icon link to the bottom of the content use a background img on an element
        const kefinRoot = window.SpaceHubConfig?.SpaceHubRoot;
        ...
        // Close original SpaceHubConfiguration Modal if it is open
        const SpaceHubConfigurationModals = document.querySelectorAll('[data-modal-id="SpaceHubConfigModal"]');
        if (SpaceHubConfigurationModals.length > 0) {
            SpaceHubConfigurationModals.forEach(modal => {
                modal.remove();
            });
        }
    ...
    async function openSpaceHubSourceModal() {
        const userIsAdmin = await isAdmin();
        if (!userIsAdmin) {
            showAlertModal('Access Denied', '<p>You must be an administrator to configure SpaceHub.</p>');
            return;
        }

        const currentConfig = await getSpaceHubConfig();
        const currentRoot = currentConfig?.SpaceHubRoot || '';
        const sourceInfo = parseSpaceHubSource(currentRoot);
        ...
        enabledCheckbox.id = 'SpaceHubEnabledModal';
        enabledCheckbox.className = 'checkbox';
        enabledCheckbox.checked = isEnabled;
        
        const enabledLabel = document.createElement('label');
        enabledLabel.setAttribute('for', 'SpaceHubEnabledModal');
        ...
        enabledText.id = 'SpaceHubEnabledModalLabel';
        ...
        sourceTypeSelect.id = 'SpaceHubSourceType';
        sourceTypeSelect.innerHTML = `
            <option value="github" ${sourceInfo.sourceType === 'github' ? 'selected' : ''}>SpaceHub GitHub</option>
        ...
        sourceSelect.id = 'SpaceHubSource';
        ...
        customUrlInput.id = 'SpaceHubCustomUrl';
        customUrlInput.placeholder = 'https://example.com/SpaceHub/';
        ...
        saveBtn.onclick = async () => {
            try {
                ...
                const SpaceHubRoot = buildSpaceHubRootUrl(sourceType, source);
                
                // Load default config from the selected root
                let defaultConfig;
                try {
                    defaultConfig = await loadDefaultConfig(SpaceHubRoot);
                } catch (error) {
                    console.error('[SpaceHub Installer] Error loading default config:', error);
                    // Create minimal config if loading fails
                    defaultConfig = {
                        SpaceHubRoot: SpaceHubRoot,
                        scripts: {},
                        homeScreen: {},
                        exclusiveElsewhere: {},
                        search: {},
                ...
                const mergedConfig = {
                    ...
                    SpaceHubRoot: SpaceHubRoot,
                    ...(currentConfig || {}),
                    SpaceHubRoot: SpaceHubRoot, // Ensure root is updated
                ...
                // Update window.SpaceHubConfig immediately so injector can use it
                window.SpaceHubConfig = mergedConfig;
                window.SpaceHubConfigEnabled = enabledState;
                
                closeSimpleModal(MODAL_ID);
                
                // Update the plugin card with new version/status
                await updateSpaceHubPluginCard();
                
                // Determine if this is initial installation
                const isInitialInstall = !currentConfig || !currentConfig.SpaceHubRoot || currentConfig.SpaceHubRoot === '';
                
                // Get version for display
                const sourceInfo = parseSpaceHubSource(SpaceHubRoot);
                let versionDisplay = '';
                if (sourceInfo.sourceType === 'custom') {
                    ...
                    try {
                        ...
                        console.warn('[SpaceHub Installer] Error fetching latest version:', error);
                        versionDisplay = LATEST_RELEASE_NAME;
                    }
                } else {
                ...
                await loadInjectorFromRoot(SpaceHubRoot);
                
                // Show success modal
                await showSuccessModal(isInitialInstall, versionDisplay);
            } catch (error) {
                console.error('[SpaceHub Installer] Error saving configuration:', error);
        ...
        // Hide uninstall button if SpaceHub isn't installed yet (no root URL)
        ...
        uninstallBtn.onclick = () => {
            ...
            showConfirmationModal(
                'Are you sure you want to uninstall SpaceHub?<br><br>This will disable the installer and backup your configuration.',
                async () => {
                    try {
                        await uninstallSpaceHub();
                        // Remove the plugin card
                        removeSpaceHubPluginCard();
                        showAlertModal(
                            'SpaceHub Uninstalled',
                            '<p>SpaceHub has been uninstalled. The installer has been disabled and your configuration has been backed up.<br><br>Please refresh your page.<br><br>You can re-enable SpaceHub at any time from the JS Injector Plugin.'
                        );
                    } catch (error) {
                        console.error('[SpaceHub Installer] Error uninstalling:', error);
                        showAlertModal(
                            'Uninstall Error',
                            `<p>Error uninstalling SpaceHub: ${error.message}</p>`
                        );
                    }
                },
                () => {
                    // User cancelled, reopen the source modal
                    openSpaceHubSourceModal();
                }
            );
        };
        ...
        createSimpleModal(MODAL_ID, 'SpaceHub Plugin Settings', content, footer);
    }

    /**
     * Uninstall SpaceHub
     * 1. Renames SpaceHub-Config to SpaceHub-Config-Backup-Timestamp
     * 2. Disables the installer script (script containing SpaceHub-plugin.js)
     */
    async function uninstallSpaceHub() {
        try {
            const pluginId = await findJavaScriptInjectorPlugin();
            const injectorConfig = await getJavaScriptInjectorConfig(pluginId);
            ...
            // Step 1: Rename SpaceHub-Config to backup
            ...
            if (configScriptIndex !== -1) {
                ...
                injectorConfig.CustomJavaScripts[configScriptIndex].Name = `SpaceHub-Config-Backup-${timestamp}`;
                injectorConfig.CustomJavaScripts[configScriptIndex].Enabled = false;
                console.log('[SpaceHub Installer] Renamed SpaceHub-Config to backup and disabled it');
            }

            // Step 2: Find and disable installer script
            // Look for script containing SpaceHub-plugin.js (flexible pattern to match script.src*SpaceHub-plugin.js)
            const installerScriptIndex = injectorConfig.CustomJavaScripts.findIndex(
                script => script.Script && /script\.src.*SpaceHub-plugin\.js/i.test(script.Script)
            );
            
            if (installerScriptIndex !== -1) {
                injectorConfig.CustomJavaScripts[installerScriptIndex].Enabled = false;
                console.log('[SpaceHub Installer] Disabled installer script');
            } else {
                console.warn('[SpaceHub Installer] Installer script not found in CustomJavaScripts');
            }
            ...
            console.log('[SpaceHub Installer] Uninstall completed successfully');
            return true;
        } catch (error) {
            console.error('[SpaceHub Installer] Error uninstalling:', error);
            throw error;
     ...
     */
    async function updateSpaceHubPluginCard() {
        const card = document.querySelector('[data-id="SpaceHubPlugin"]');
        if (!card) {
            ...
            if (pluginsPage) {
                ...
                if (installedPlugins) {
                    addSpaceHubPluginCard(installedPlugins);
                }
            }
            return;
        }
        ...
        try {
            ...
            const root = config?.SpaceHubRoot || '';
            const sourceInfo = parseSpaceHubSource(root);
            ...
            if (isMuiCard) {
                ...
                if (cardTextElements.length > 0) {
                    ...
                    if (versionSpan) {
                        ...
                        nameElement.innerHTML = `SpaceHub<span class="cardText cardText-secondary">${versionDisplay}</span>`;
                    }
                }
            ...
            console.error('[SpaceHub Installer] Error updating plugin card:', error);
        }
    }
     ...
     */
    function removeSpaceHubPluginCard() {
        const card = document.querySelector('[data-id="SpaceHubPlugin"]');
        if (card) {
            card.remove();
            console.log('[SpaceHub Installer] Plugin card removed');
        }
    }

    // Make function globally accessible
    window.openSpaceHubSourceModal = openSpaceHubSourceModal;

    // Add plugin card to plugins page
    function addSpaceHubPluginCard(installedPlugins) {
        if (!installedPlugins) {
            return;
        }

        // Check if card already exists in either container
        if (document.querySelector('[data-id="SpaceHubPlugin"]')) {
            return;
        }
        ...
        if (!existingCard) {
            ...
            console.log('[SpaceHub Installer] No existing plugin card found, setting up MutationObserver');
            ...
            const observer = new MutationObserver((mutations) => {
                ...
                for (const mutation of mutations) {
                    if (mutation.addedNodes.length > 0) {
                        ...
                        if (cardToClone) {
                            console.log('[SpaceHub Installer] Plugin card detected, disconnecting observer');
                            observer.disconnect();
            ...
            setTimeout(() => {
                observer.disconnect();
                console.warn('[SpaceHub Installer] MutationObserver timeout: No plugin cards appeared after 10 seconds');
            }, 10000);
        ...
        getSpaceHubConfig().then(async config => {
            const root = config?.SpaceHubRoot || '';
            const sourceInfo = parseSpaceHubSource(root);
            ...
            // Update data attributes
            card.setAttribute('data-id', 'SpaceHubPlugin');
            card.setAttribute('data-version', versionDisplay);
            card.setAttribute('data-name', 'SpaceHub');
            ...
            if (isMuiCard) {
                ...
                if (nameElement) {
                    nameElement.textContent = 'SpaceHub';
                }
                ...
                if (cardTextElements.length > 0) {
                    ...
                    if (versionSpan) {
                        ...
                        if (nameElement.firstChild && nameElement.firstChild.nodeType === Node.TEXT_NODE) {
                            nameElement.firstChild.textContent = 'SpaceHub';
                        } else {
                            // Insert name text before the version span
                            nameElement.insertBefore(document.createTextNode('SpaceHub'), versionSpan);
                        }
                    } else {
                        // If no secondary span exists, create one
                        nameElement.innerHTML = `SpaceHub<span class="cardText cardText-secondary">${versionDisplay}</span>`;
            ...
            console.error('[SpaceHub Installer] Error getting config for card:', err);
        });
    }
    ...
    function checkForPluginsPage(view, element, hash) {
        ...
        if (hash && hash.includes('dashboard/plugins')) {
            const pluginsPage = document.querySelector('#pluginsPage:not(.hide)');
            if (!pluginsPage) {
                console.log('[SpaceHub Installer] Plugins page not found');
                return;
            }
            ...
            if (!installedPlugins || !pluginsPage) {
                console.log('[SpaceHub Installer] Installed plugins or plugins page not found');
                return;
            }
        ...
        try {
            ...
            if (!window.ApiClient || !window.ApiClient._loggedIn) {
                if (retryCount < maxRetries) {
                    ...
                    console.log('[SpaceHub Installer] User not logged in after polling, skipping setup');
                    return;
                }
            }
            ...
            const userIsAdmin = await isAdmin();
            if (!userIsAdmin) {
                console.log('[SpaceHub Installer] User is not admin, skipping setup');
                return;
            }

            // User is logged in and is admin, proceed with setup
            console.log('[SpaceHub Installer] User is admin, setting up functionality');

            // Add CSS
            const css = document.createElement('style');
            css.textContent = `
                [data-id="SpaceHubPlugin"] .cardImageContainer::after,
                [data-id="SpaceHubPlugin"] .MuiButtonBase-root.MuiCardActionArea-root::after {
                    content: 'SpaceHub';
                    position: absolute;
                    font-size: 2em;
                    top: 50%;
            ...
            console.error('[SpaceHub Installer] Error checking user:', error);
            // Retry on error
            if (retryCount < maxRetries) {
                setTimeout(() => {
                    checkUserAndSetup(retryCount + 1);
    ...
    function setupOnViewShow() {
        ...
        if (window.Emby && window.Emby.Page) {
            window.Emby.Page.onViewShow = function(...args) {
                ...
                if (originalOnViewShow) {
                    try {
                        ...
                        console.warn('[SpaceHub Installer] Error in original onViewShow:', err);
                    }
                }
            ...
            console.log('[SpaceHub Installer] Hooked into Emby.Page.onViewShow');
        } else {
    ...
    async function loadInjectorFromRoot(root) {
        if (!root || root === '') {
            console.warn('[SpaceHub Installer] Cannot load injector: SpaceHubRoot is empty');
            return false;
        }
        ...
        if (existingScript) {
            console.log('[SpaceHub Installer] Injector already loaded from:', injectorUrl);
            return true;
        }

        console.log('[SpaceHub Installer] Loading injector.js from:', injectorUrl);
        ...
        return new Promise((resolve, reject) => {
            ...
            script.onload = () => {
                console.log('[SpaceHub Installer] Injector loaded successfully');
                resolve(true);
            };
            
            script.onerror = (error) => {
                console.error('[SpaceHub Installer] Failed to load injector:', error);
    ...
    async function checkAndLoadInjector() {
        try {
            ...
            if (root && root !== '') {
                ...
                console.log('[SpaceHub Installer] SpaceHubRoot not configured. Please configure via the Plugins page.');
            }
        } catch (error) {
            console.error('[SpaceHub Installer] Error checking config:', error);
        }
    }
    ...
    console.log('[SpaceHub Installer] Installer script loaded');
})();
