...
// P0 — Extrait en fonction nommée (au lieu d'être inline dans configureServer) afin de
// pouvoir être branchée à la fois sur `vite dev` (configureServer) ET `vite preview`
// (configurePreviewServer). Avant ce correctif, `/api-proxy` n'existait QUE sous `vite dev` :
// en méthode 2 (self-host), lancer `npm run build` puis servir `dist/` avec `vite preview`,
// nginx ou Docker faisait disparaître le proxy sans avertissement, cassant silencieusement
// toute intégration (Sonarr/Radarr/Prowlarr/Bazarr/Jellyseerr/qBittorrent) dès qu'un
// service cible ne renvoie pas d'en-têtes CORS. Voir docs/ARCHITECTURE.md pour le détail.
function createCorsProxyMiddleware() {
  return (req, res) => {
...
function dynamicCorsProxyPlugin() {
  return {
    ...
    configureServer(server) {
      server.middlewares.use('/api-proxy', createCorsProxyMiddleware());
    },
    // `vite preview` — P0 : sans ce hook, `npm run build && npm run preview` (méthode 2
    // self-host) ne servait pas `/api-proxy`, contrairement à `npm run dev`.
