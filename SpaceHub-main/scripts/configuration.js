...
function escapeHtml(str) {
    ...
    async function isAdmin() {
        try {
            ...
            console.warn('[SpaceHub Configuration] Could not check admin status:', error);
        }
        return false;
    }
    ...
    async function loadDefaultConfig() {
        return new Promise((resolve, reject) => {
            try {
                const defaultConfigUrl = window.SpaceHubConfig?.SpaceHubRoot 
                    ? `${window.SpaceHubConfig.SpaceHubRoot}/SpaceHub-default-config.js` : null;
                ...
                if (window.SpaceHubDefaultConfig) {
                    console.log('[SpaceHub Configuration] Using cached default config');
                    resolve(window.SpaceHubDefaultConfig);
                    return;
                }
                ...
                script.onload = () => {
                    if (window.SpaceHubDefaultConfig) {
                        console.log('[SpaceHub Configuration] Loaded default config:', window.SpaceHubDefaultConfig);
                        resolve(window.SpaceHubDefaultConfig);
                    } else {
                        reject(new Error('Default config file loaded but window.SpaceHubDefaultConfig is not defined'));
                    }
                };
                ...
                console.error('[SpaceHub Configuration] Error loading default config:', error);
                reject(error);
            }
        });
    }

    // Get SpaceHub configuration from JS Injector or load defaults
    async function getSpaceHubConfig() {
        try {
            ...
            // Find SpaceHub-Config script
            const SpaceHubScript = injectorConfig.CustomJavaScripts?.find(
                script => script.Name === 'SpaceHub-Config'
            );
            
            if (SpaceHubScript && SpaceHubScript.Script) {
                // Extract config from script content
                // The script should contain: window.SpaceHubConfig = {...};
                const scriptMatch = SpaceHubScript.Script.match(/window\.SpaceHubConfig\s*=\s*({[\s\S]*});/);
                if (scriptMatch && scriptMatch[1]) {
                    try {
                        const config = JSON.parse(scriptMatch[1]);
                        // Store enabled state globally for use in modal (defaults to true if not set)
                        window.SpaceHubConfigEnabled = config.enabled !== false;
                        console.log('[SpaceHub Configuration] Loaded config from JS Injector:', config);
                        return config;
                    } catch (parseError) {
                        console.error('[SpaceHub Configuration] Error parsing config from script:', parseError);
                    }
                }
        }

            // If no config found in JS Injector, load defaults
            console.log('[SpaceHub Configuration] No config found in JS Injector, loading defaults');
        const rawDefaultConfig = await loadDefaultConfig();
        const defaultConfig = JSON.parse(JSON.stringify(rawDefaultConfig));

        const currentConfigSource = currentLoadedConfig || window.SpaceHubCurrentConfig || window.SpaceHubConfig || {};
        const currentKefinRoot = currentConfigSource.SpaceHubRoot || defaultConfig.SpaceHubRoot;
        defaultConfig.SpaceHubRoot = currentKefinRoot;
        ...
        window.SpaceHubConfigEnabled = defaultConfig.enabled !== false;
        // Note: scriptRoot is no longer used - scripts are loaded from SpaceHubRoot + '/scripts/'

            // Save defaults to JS Injector for future use
            await SpaceHubUtils.saveConfigToJavaScriptInjector(defaultConfig);
            
            return defaultConfig;
        } catch (error) {
            console.error('[SpaceHub Configuration] Error getting config, falling back to defaults:', error);
            // Fallback to defaults if anything fails
            try {
                return await loadDefaultConfig();
            } catch (fallbackError) {
                console.error('[SpaceHub Configuration] Error loading default config as fallback:', fallbackError);
    ...
    function injectConfigModalCSS() {
        const styleId = 'SpaceHub-config-modal-css';
    ...
    function buildConfigPageContent(config, libraries = []) {
        ...
        // We can't use window.SpaceHubSkinConfig directly because it filters out disabled skins
        const defaultSkins = window.SpaceHubDefaultSkinsConfig?.skins || [];
        ...
        const skinSources = window.SpaceHubSkinManager?.getAllSkinSources?.() || {};

        const isEnabled = window.SpaceHubConfigEnabled !== false;
                ...
                <div class="listItem">
                    <div class="listItemContent" style="display: flex; gap: 1em; align-items: center; flex-wrap: wrap;">
                        <label class="checkboxContainer" style="display: flex; align-items: center; gap: 0.75em; cursor: pointer;">
                            <input type="checkbox" id="SpaceHubEnabled" class="checkbox" ${isEnabled ? 'checked' : ''}>
                            <span id="SpaceHubEnabledLabel" class="listItemBodyText" style="margin: 0;">${isEnabled ? 'Enabled' : 'Disabled'}</span>
                        </label>
                        <button class="emby-button raised" id="changeSpaceHubSourceBtn" style="padding: 0.75em 2em; font-size: 1em;" title="Switch between the Latest, Development, Version specific branches or point to your own self hosted location!">
            ...
            <div class="paperList" style="margin-bottom: 2em;">
                <div class="listItem" style="border-bottom: 1px solid rgba(255,255,255,0.1); padding-bottom: 1em; margin-bottom: 1em;">
                    <div class="listItemContent">
                        ...
                        <div class="listItemBodyText secondary">Toggle individual SpaceHub features on or off. Existing configurations are maintained when disabling features.</div>
                    </div>
                </div>
            ...
            <div class="paperList" id="configSection_skin" style="margin-bottom: 2em; ${scripts.skinManager === false ? 'display: none;' : ''}">
                ...
                <div class="listItem" style="border: 1px solid rgba(255,255,255,0.1); border-radius: 4px; padding: 0.75em; margin-bottom: 1em;">
                    <div class="listItemContent" style="width: 100%;">
                        ...
                        <div class="listItemBodyText secondary" style="margin-bottom: 1em; font-size: 0.85em; color: rgba(255,255,255,0.7); line-height: 1.5;">
                            ...
                            SpaceHub has not contributed to the creation of any of the skins below and is only attempting to facilitate their accessibility.
                        </div>
                ...
                <details class="listItem" style="border: 1px solid rgba(255,255,255,0.1); border-radius: 4px; padding: 0; margin-bottom: 1em; display: grid;">
                    ...
                    <div style="padding: 0.75em; border-top: 1px solid rgba(255,255,255,0.1);">
                        <div class="listItemContent">
                            <div class="listItemBodyText secondary" style="margin-bottom: 0.75em; font-size: 0.9em;">Add additional skins that will be available to all users. Each skin can have multiple CSS files for different server versions. You can override the Default skins from SpaceHub by specifying a custom configuration with that Skin Name in the configuration JSON below. Any skins that appear in this JSON and are named the same as the SpaceHub default skins will override the default functionality. Default skins are not shown here as they are managed separately.</div>
  ...
  },
  {
     ...
     * @param {Object} config - The SpaceHubConfig object
    ...
    function buildOptionalIncludesEditor(category, config, skins) {
        ...
        if (category === 'global') {
            ...
            const defaultGlobalIncludes = window.SpaceHubDefaultSkinsConfig?.globalOptionalIncludes || [];
    ...
    function buildSkinToggles(skins, skinSources = {}) {
        return skins.map(skin => {
            ...
            let tooltipText = '';
            if (source === 'default') {
                tooltipText = 'This skin is included automatically by SpaceHub';
            } else if (source === 'overridden') {
    ...
    async function fetchLibraries() {
        ...
        try {
            ...
            console.error('[SpaceHub Configuration] Error fetching libraries:', error);
            return [];
        }
    }

    async function openConfigurationModal() {
        console.log('[SpaceHub Configuration] Opening configuration modal...');
        ...
        let config;
        try {
            config = await getSpaceHubConfig();
        } catch (error) {
            console.error('[SpaceHub Configuration] Error loading config:', error);
            alert('Error loading configuration. Please try again.');
            return;
        }

        currentLoadedConfig = config;
        window.SpaceHubCurrentConfig = config;
        ...
        if (!window.ModalSystem) {
            console.error('[SpaceHub Configuration] ModalSystem not available');
    ...
    function setupConfigModalHandlers(modalInstance, config) {
        ...
        const enabledCheckbox = modalInstance.dialogContent.querySelector('#SpaceHubEnabled');
        const enabledLabel = modalInstance.dialogContent.querySelector('#SpaceHubEnabledLabel');
        if (enabledCheckbox) {
            ...
            enabledCheckbox.addEventListener('change', async (e) => {
                ...
                try {
                    ...
                    await SpaceHubUtils.saveConfigToJavaScriptInjector(config);
                    
                    window.SpaceHubConfigEnabled = isEnabled;
                    console.log('[SpaceHub Configuration] SpaceHub', isEnabled ? 'enabled' : 'disabled');
                    
                    if (window.SpaceHubToaster && window.SpaceHubToaster.toast) {
                        window.SpaceHubToaster.toast(`SpaceHub ${isEnabled ? 'enabled' : 'disabled'}. Page refresh required.`);
                    }
                } catch (error) {
                    console.error('[SpaceHub Configuration] Error toggling SpaceHub:', error);
                    // Revert checkbox on error
                    enabledCheckbox.checked = !isEnabled;
                    updateLabel();
                    alert(`Failed to ${isEnabled ? 'enable' : 'disable'} SpaceHub: ${error.message}`);
                }
            });
        }
        
        // Change SpaceHub Source button handler
        const changeSourceBtn = modalInstance.dialogContent.querySelector('#changeSpaceHubSourceBtn');
        if (changeSourceBtn) {
            changeSourceBtn.addEventListener('click', async () => {
                try {
                    // Check if the modal function is available
                    if (window.openSpaceHubSourceModal) {
                        // Open the SpaceHub source modal
                        window.openSpaceHubSourceModal();
                    } else {
                        throw new Error('SpaceHub source modal is not available. Please ensure SpaceHub-plugin.js is loaded.');
                    }
                } catch (error) {
                    console.error('[SpaceHub Configuration] Error opening source modal:', error);
        ...
        modalInstance.dialogContent.addEventListener('click', (e) => {
            ...
            if (deleteBtn) {
                e.preventDefault();
                e.stopPropagation();
                console.log('[SpaceHub Configuration] Checking for section to delete...');
                ...
                console.log('[SpaceHub Configuration] Prefix:', prefix);
                console.log('[SpaceHub Configuration] Section Index Raw:', sectionIndexRaw);
                console.log('[SpaceHub Configuration] Is Seasonal:', isSeasonal);
                if (prefix && sectionIndexRaw !== null) {
                    console.log('[SpaceHub Configuration] Deleting section...');
        ...
        function deleteSection(prefix, sectionIndex, isSeasonalNested) {
            console.log('[SpaceHub Configuration] Deleting section:', prefix, sectionIndex, isSeasonalNested);
        ...
        async function updateSectionSourceField(prefix, sectionIndex, isSeasonalNested) {
            ...
            if (!sectionItem) {
                console.warn(`[SpaceHub Configuration] Section item not found for prefix: ${prefix}, index: ${sectionIndex}`);
                return;
            }
            
            const typeSelect = sectionItem.querySelector(`.${prefix}_section_type[data-section-index="${sectionIndex}"]`);
            if (!typeSelect) {
                console.warn(`[SpaceHub Configuration] Type select not found for prefix: ${prefix}, index: ${sectionIndex}`);
                return;
            }
            ...
            if (!sourceContainer) {
                console.warn(`[SpaceHub Configuration] Source container not found for prefix: ${prefix}, index: ${sectionIndex}`);
                return;
            }
        ...
        async function setupBadgeAutocomplete(prefix, sectionIndex, type) {
            ...
            try {
                ...
                console.error('[SpaceHub Configuration] Error fetching options:', err);
            }
        ...
        async function loadBadgeNames(prefix, sectionIndex, type) {
            ...
            try {
                ...
                console.error('[SpaceHub Configuration] Error loading badge names:', err);
        ...
        window.openCollectionPicker = async function(seasonIndex) {
            ...
            try {
                ...
                console.error('[SpaceHub Configuration] Error opening collection picker:', error);
        ...
        window.openPlaylistPicker = async function(seasonIndex) {
            ...
            try {
                ...
                console.error('[SpaceHub Configuration] Error opening playlist picker:', error);
        ...
        async function openCollectionPickerForSection(prefix, sectionIndex, isSeasonalNested) {
            ...
            try {
                ...
                console.error('[SpaceHub Configuration] Error opening collection picker for section:', error);
                alert('Error loading collections. Please try again.');
            }
        };
        ...
        async function openPlaylistPickerForSection(prefix, sectionIndex, isSeasonalNested) {
            ...
            try {
                ...
                console.error('[SpaceHub Configuration] Error opening playlist picker for section:', error);
                alert('Error loading playlists. Please try again.');
            }
        };
        ...
        if (optionalIncludesCategory) {
            optionalIncludesCategory.addEventListener('change', (e) => {
                ...
                if (editor) {
                    ...
                    const currentConfig = { 
                        ...(window.SpaceHubConfig || config), 
                        optionalIncludes: pendingOptionalIncludes 
                    };
                    
                    editor.innerHTML = buildOptionalIncludesEditor(newCategory, currentConfig, window.SpaceHubSkinConfig || []);
    ...
    function handleRemoveDuplicateDefaults(modalInstance) {
        ...
        try {
            ...
            const defaultSkinNames = new Set((window.SpaceHubDefaultSkinsConfig?.skins || []).map(s => s.name));
            ...
            if (removedCount === 0) {
                if (window.SpaceHubToaster && window.SpaceHubToaster.toast) {
                    window.SpaceHubToaster.toast('No duplicate default skins found in configuration');
                } else {
            ...
            // Show confirmation
            if (window.SpaceHubToaster && window.SpaceHubToaster.toast) {
                window.SpaceHubToaster.toast(`Removed ${removedCount} custom skin${removedCount > 1 ? 's' : ''} that were overriding defaults`);
            } else {
                alert(`Removed ${removedCount} custom skin${removedCount > 1 ? 's' : ''} that were overriding defaults`);
            }
        } catch (error) {
            console.error('[SpaceHub Configuration] Error removing duplicate defaults:', error);
            if (window.SpaceHubToaster && window.SpaceHubToaster.toast) {
                window.SpaceHubToaster.toast('Error processing skins JSON', '5');
            } else {
    ...
    async function handleExportConfig(config) {
        try {
            ...
            if (window.SpaceHubToaster && window.SpaceHubToaster.toast) {
                window.SpaceHubToaster.toast('Configuration exported to clipboard!');
            } else {
                alert('Configuration exported to clipboard!');
            }
        } catch (error) {
            console.error('[SpaceHub Configuration] Error exporting config:', error);
            
            if (window.SpaceHubToaster && window.SpaceHubToaster.toast) {
                window.SpaceHubToaster.toast('Error exporting configuration to clipboard', '5');
            } else {
            ...
            onOpen: (importModalInstance) => {
                ...
                if (confirmBtn) {
                    confirmBtn.addEventListener('click', async () => {
                        ...
                        if (!jsonText) {
                            if (window.SpaceHubToaster && window.SpaceHubToaster.toast) {
                                window.SpaceHubToaster.toast('Please paste configuration JSON', '3');
                            } else {
                        ...
                        try {
                            ...
                            console.error('[SpaceHub Configuration] Invalid JSON:', parseError);
                            if (window.SpaceHubToaster && window.SpaceHubToaster.toast) {
                                window.SpaceHubToaster.toast(`Invalid JSON: ${parseError.message}`, '5');
                            } else {
                        ...
                        if (typeof importedConfig !== 'object' || importedConfig === null) {
                            if (window.SpaceHubToaster && window.SpaceHubToaster.toast) {
                                window.SpaceHubToaster.toast('Invalid configuration format', '5');
                            } else {
                        ...
                        try {
                            ...
                            console.error('[SpaceHub Configuration] Error applying imported config:', error);
                            if (window.SpaceHubToaster && window.SpaceHubToaster.toast) {
                                window.SpaceHubToaster.toast(`Error applying configuration: ${error.message}`, '5');
                            } else {
    ...
    async function applyImportedConfig(importedConfig, modalInstance) {
        ...
        await SpaceHubUtils.saveConfigToJavaScriptInjector(importedConfig);
        ...
        if (window.SpaceHubToaster && window.SpaceHubToaster.toast) {
            window.SpaceHubToaster.toast('Configuration imported successfully! Reloading...');
        }
    ...
    async function findJavaScriptInjectorPlugin() {
        try {
            ...
            console.log('[SpaceHub Configuration] Active Plugins:', plugins);
            ...
            console.log('[SpaceHub Configuration] Found JavaScript Injector plugin:', injectorPlugin);
            return injectorPlugin.Id;
        } catch (error) {
            console.error('[SpaceHub Configuration] Error finding JavaScript Injector plugin:', error);
            throw error;
        }
    }
    ...
    async function getJavaScriptInjectorConfig(pluginId) {
        try {
            ...
            console.log('[SpaceHub Configuration] Current JS Injector config:', config);
            return config;
        } catch (error) {
            console.error('[SpaceHub Configuration] Error getting JS Injector config:', error);
            throw error;
        }
    }
    ...
    async function fetchItemsForSectionPreview(sectionConfig) {
        ...
        try {
            ...
            console.warn('[SpaceHub Configuration] Preview fetch failed:', error);
            throw error;
        }
    }
    ...
    async function handleSaveConfig(modalInstance, overrideOptionalIncludes = null) {
        ...
        console.log('[SpaceHub Configuration] Saving configuration...');
        
        // Collect all form values
        // Preserve existing SpaceHubRoot from current config (no longer editable in UI)
        const currentConfig = window.SpaceHubCurrentConfig || {};
        const config = {
            SpaceHubRoot: currentConfig.SpaceHubRoot || '',
            scripts: {},
            homeScreen: {},
            exclusiveElsewhere: {},
            search: {},
        ...
        config.enabled = modalInstance.dialogContent.querySelector('#SpaceHubEnabled')?.checked !== false;
        ...
        if (overrideOptionalIncludes) {
            ...
            console.warn('[SpaceHub Configuration] handleSaveConfig called without overrideOptionalIncludes - preserving existing optionalIncludes');
        ...
        console.log('[SpaceHub Configuration] Configuration collected:', config);
        ...
        try {
            ...
            // Show success toast
            if (window.SpaceHubToaster && window.SpaceHubToaster.toast) {
                window.SpaceHubToaster.toast('SpaceHub saved!');
            }
        } catch (error) {
            console.error('[SpaceHub Configuration] Error saving configuration:', error);
            
            // Show error toast
            if (window.SpaceHubToaster && window.SpaceHubToaster.toast) {
                window.SpaceHubToaster.toast(`Error saving configuration: ${error.message}. Please ensure the JavaScript Injector plugin is installed and you have administrator permissions.`, '5');
            } else {
    ...
    async function handleResetConfig(modalInstance) {
        ...
        const confirmed = confirm('This will restore the SpaceHub configuration to its defaults. It cannot be undone. Are you sure you want to continue?');
        if (!confirmed) {
            return;
        }
        ...
        try {
            ...
            // Retain SpaceHubRoot from existing config
            const existingConfig = await getSpaceHubConfig();
            defaultConfig.SpaceHubRoot = existingConfig.SpaceHubRoot;
            // Note: scriptRoot is no longer used - scripts are loaded from SpaceHubRoot + '/scripts/'

            await SpaceHubUtils.saveConfigToJavaScriptInjector(defaultConfig);

            if (window.SpaceHubToaster && window.SpaceHubToaster.toast) {
                window.SpaceHubToaster.toast('Configuration reset to defaults.');
            } else {
            ...
            console.error('[SpaceHub Configuration] Error resetting config:', error);
            if (window.SpaceHubToaster && window.SpaceHubToaster.toast) {
                window.SpaceHubToaster.toast(`Error resetting configuration: ${error.message}`, '5');
            } else {
    ...
    async function handleResetAllUsersConfig(modalInstance) {
        ...
        const confirmed = confirm('WARNING: This will force ALL users to reset their SpaceHub settings to the default configuration on their next visit. This action cannot be undone. Are you sure you want to continue?');
        if (!confirmed) {
            return;
        }

        try {
            const config = await getSpaceHubConfig();
            ...
            if (window.SpaceHubToaster && window.SpaceHubToaster.toast) {
                window.SpaceHubToaster.toast('Reset signal sent. All users will be reset on next load.');
            } else {
                alert('Reset signal sent. All users will be reset on next load.');
            }
        } catch (error) {
            console.error('[SpaceHub Configuration] Error resetting all users config:', error);
            if (window.SpaceHubToaster && window.SpaceHubToaster.toast) {
                window.SpaceHubToaster.toast(`Error resetting configuration: ${error.message}`, '5');
            } else {
                alert(`Error resetting configuration: ${error.message}`);
    ...
    function parseJSONField(fieldId, defaultValue) {
        try {
            ...
            console.error(`[SpaceHub Configuration] Error parsing JSON field ${fieldId}:`, error);
    ...
    function createConfigButton() {
        ...
        button.setAttribute('data-SpaceHub-config-button', 'true');
        button.onclick = (e) => {
            e.preventDefault();
            openConfigurationModal();
        };
        ...
        button.innerHTML = `
            <div class="listItem">
                ...
                <div class="listItemBody">
                    <div class="listItemBodyText">Configure SpaceHub</div>
                </div>
            </div>
        `;
    ...
    function createDashboardConfigButton() {
        ...
        if (document.querySelector('[data-SpaceHub-dashboard-config-button]')) {
            return null;
        }
        ...
        div.setAttribute('data-SpaceHub-dashboard-config-button', 'true');
        div.onclick = () => {
            openConfigurationModal();
        };
    ...
    function addConfigButtonToDashboard() {
        ...
        if (!pluginsSection) {
            console.log('[SpaceHub Configuration] Plugins section not found, retrying...');
        ...
        console.log('[SpaceHub Configuration] Configuration button added to dashboard side menu');
    }
    ...
    function addConfigButtonToAdminSection() {
        ...
        if (document.querySelector('.adminSection [data-SpaceHub-config-button]')) {
            return;
        }
        ...
        if (!adminSection) {
            console.log('[SpaceHub Configuration] Administration section not found, retrying...');
        ...
        console.log('[SpaceHub Configuration] Configuration button added to Administration section');
    }

    // Register onViewPage handler for mypreferencesmenu page and dashboard
    function registerViewPageHandler() {
        if (!window.SpaceHubUtils || !window.SpaceHubUtils.onViewPage) {
            console.log('[SpaceHub Configuration] SpaceHubUtils.onViewPage not available, retrying...');
            setTimeout(registerViewPageHandler, 1000);
            return;
        }

        // Handler for dashboard and preferences pages
        window.SpaceHubUtils.onViewPage(async (view, element, hash) => {
            // Check if user is admin before adding buttons
            const userIsAdmin = await isAdmin();
            if (!userIsAdmin) {
                console.log('[SpaceHub Configuration] User is not admin, not adding buttons');
                return;
            }
            ...
            if (hash && (hash.includes('mypreferencesmenu') || hash.includes('userpreferences') || hash.includes('preferences'))) {
                console.log('[SpaceHub Configuration] Preferences page detected');
            ...
            if (view === 'dashboard' || view === 'configurationpage' || hash.includes('dashboard') || hash.includes('configurationpage')) {
                console.log('[SpaceHub Configuration] Dashboard page detected');
                // Wait a bit for the page to fully load
                    setTimeout(() => {
        ...
        console.log('[SpaceHub Configuration] Registered onViewPage handler');
    }
    ...
    async function addConfigLinkToCustomMenu() {
        ...
        const userIsAdmin = await isAdmin();
        if (!userIsAdmin) {
            console.log('[SpaceHub Configuration] User is not admin, not adding menu link');
            return;
        }

        // Check if utils is available
        if (!window.SpaceHubUtils || !window.SpaceHubUtils.addCustomMenuLink) {
            console.log('[SpaceHub Configuration] SpaceHubUtils.addCustomMenuLink not available, retrying...');
        ...
        // Add the menu link
        const success = await window.SpaceHubUtils.addCustomMenuLink(
            'Configure',
            'build',
            configUrl,
            false,
        ...
        if (success) {
            console.log('[SpaceHub Configuration] Configuration link added to custom menu');
    ...
    function setupConfigLinkClickHandler() {
        ...
        if (!configButton) {
            console.log('[SpaceHub Configuration] Configuration button not found, retrying...');
    ...
    function handleDirectConfigNavigation() {
        if (window.location.hash === '#SpaceHub-config') {
            console.log('[SpaceHub Configuration] Direct navigation to config URL detected');
    ...
    function setupHashChangeListener() {
        window.addEventListener('hashchange', () => {
            if (window.location.hash === '#SpaceHub-config') {
                console.log('[SpaceHub Configuration] Hash change to config URL detected');
                // Remove the hash to prevent navigation
                window.history.replaceState(null, '', window.location.pathname + window.location.search);
                // Open modal
                openConfigurationModal();
    ...
    console.log('[SpaceHub Configuration] Script loaded');
})();
