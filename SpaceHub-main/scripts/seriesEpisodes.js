...
(function() {
    ...
    const LOG = (...args) => console.log('[SpaceHub SeriesEpisodes]', ...args);
    const WARN = (...args) => console.warn('[SpaceHub SeriesEpisodes]', ...args);
    const ERR = (...args) => console.error('[SpaceHub SeriesEpisodes]', ...args);
    ...
    function createSeasonSelector(sectionTitleContainer, seasons, currentSeason, onSeasonSelect) {
        ...
        const createPopover = () => {
            ...
            popover.className = 'SpaceHub-popover seasonPopover itemDetailsGroup';
    ...
    async function renderEpisodesSection(seriesId, seasons, targetEpisode, nextUpHeaderText) {
        ...
        const hideSingleSeasonContainer = window.SpaceHubConfig?.flattenSingleSeasonShows?.hideSingleSeasonContainer === true;
        ...
        if (targetEpisodeNumber && scrollerContainer) {
            if (targetEpisodeNumber.season === targetSeason.indexNumber) {
                ...
                if (!document.getElementById('SpaceHub-nextUpEpisode-style')) {
                    const style = document.createElement('style');
                    style.id = 'SpaceHub-nextUpEpisode-style';
    ...
    function initializeSeriesEpisodesHook() {
        ...
        LOG('Registering series episodes handler with SpaceHubUtils');

        window.SpaceHubUtils.onViewPage(
    ...
    function initialize() {
        if (!window.SpaceHubUtils || !window.SpaceHubUtils.onViewPage) {
            WARN('SpaceHubUtils not available, retrying in 1 second');
