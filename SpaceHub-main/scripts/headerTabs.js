...
(function() {
    ...
    const LOG = (...args) => console.log('[SpaceHub HeaderTabs]', ...args);
    const WARN = (...args) => console.warn('[SpaceHub HeaderTabs]', ...args);
    const ERR = (...args) => console.error('[SpaceHub HeaderTabs]', ...args);
    ...
    function updateUrlWithTab(tabIndex) {
        ...
        window.SpaceHubUtils.notifyHandlers(currentPage, document, newHash);
    ...
    async function addClickListeners() {
        ...
        if (headerTabs) {
            ...
            const view = window.SpaceHubUtils.getCurrentView();
    ...
    function initialize() {
        if (!window.SpaceHubUtils || !window.SpaceHubUtils.onViewPage) {
            ...
            if (INIT_ATTEMPTS < MAX_INIT_ATTEMPTS) {
                ...
                ERR('SpaceHubUtils not available after 10 seconds, giving up');
            }
            return;
        }
