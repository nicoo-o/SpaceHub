...
(function() {
    ...
    // Wait for SpaceHubUtils to become available
    async function waitForUtils(maxWaitTime = 30000, checkInterval = 100) {
        ...
        while (Date.now() - startTime < maxWaitTime) {
            if (window.SpaceHubUtils && window.SpaceHubUtils.onViewPage) {
                LOG('SpaceHubUtils is now available');
                return true;
            }
        ...
        ERR('SpaceHubUtils not available after waiting. Device Manager requires utils.js');
        return false;
    }
