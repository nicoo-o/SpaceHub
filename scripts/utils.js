...
(function() {
    ...
    const LOG = (...args) => console.log('[SpaceHub Utils]', ...args);
    const WARN = (...args) => console.warn('[SpaceHub Utils]', ...args);
    const ERR = (...args) => console.error('[SpaceHub Utils]', ...args);
    ...
    /**
     * Saves the current SpaceHubConfig back to JS Injector plugin
     * @param {Object} config - Optional config object to save. If not provided, uses window.SpaceHubConfig
    ...
    async function saveConfigToJavaScriptInjector(config = null, options = {}) {
        try {
            ...
            // Use provided config or fall back to window.SpaceHubConfig
            const configToSave = config || window.SpaceHubConfig;
            if (!configToSave) {
                throw new Error('No config provided and window.SpaceHubConfig is not available');
            }
...
// This file is automatically generated by SpaceHub Configuration UI
// Do not edit manually unless you know what you're doing

window.SpaceHubConfig = ${JSON.stringify(configToSave, null, 2)};`;
            
            // Find or create SpaceHub-Config script
	...
	async function getWatchlistTabIndex() {
        ...
        const storedTabIndex = localStorage.getItem(`SpaceHub_watchlistTabIndex_${ApiClient.serverId()}`);
        if (storedTabIndex) {
        ...
        localStorage.setItem(`SpaceHub_watchlistTabIndex_${ApiClient.serverId()}`, _watchlistTabIndex);
		return _watchlistTabIndex;
	}

    // Expose utilities to global scope
    window.SpaceHubUtils = {
        onViewPage,
        notifyHandlers,
        getCurrentView,
        getHandlerCount,
    ...
    LOG('Available at window.SpaceHubUtils');
})();
