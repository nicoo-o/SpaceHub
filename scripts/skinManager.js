...
(function() {
    ...
    const LOG = (...args) => console.log('[SpaceHub SkinManager]', ...args);
    const WARN = (...args) => console.warn('[SpaceHub SkinManager]', ...args);
    const ERR = (...args) => console.error('[SpaceHub SkinManager]', ...args);
    ...
    let SKINS_CONFIG = [];
    let THEMES_CONFIG = [];
    const STORAGE_KEY = 'SpaceHub_selectedSkin';
    const THEME_STORAGE_KEY = 'SpaceHub_selectedTheme';
    const COLOR_SCHEMES_STORAGE_KEY = 'SpaceHub_selectedColorScheme';
     ...
     * URLs pointing to /SpaceHub/skins/{filename}.css are normalized to just the filename
     ...
     * @returns {string} Normalized URL (filename if it's a SpaceHub skin, otherwise original URL)
     */
    ...
    /**
     * Create a backup of the current SpaceHubConfig in JS Injector
    ...
    async function createConfigBackup(configToBackup) {
        try {
            ...
            const backupName = `SpaceHub-Config-[SkinConflictsRemoved-${timestamp}]`;
            
            // Create the backup script content
            const backupScriptContent = `// SpaceHub Configuration Backup
// This backup was automatically created when skin conflicts were removed
// Created: ${new Date().toISOString()}
// Do not edit manually

window.SpaceHubConfig = ${JSON.stringify(configToBackup, null, 2)};`;
    ...
    async function loadSkinConfig() {
        ...
        const legacyDefaults = window.SpaceHubLegacySkinDefaults?.skins || [];
        
        // Load admin-configured skins from window.SpaceHubConfig (from JS Injector)
        let adminSkins = window.SpaceHubConfig?.skins || [];
        ...
        if (skinsToRemove.size > 0 && window.SpaceHubConfig) {
            // Create a deep copy of the original config for backup
            const originalConfig = JSON.parse(JSON.stringify(window.SpaceHubConfig));
            ...
            // Update SpaceHubConfig with cleaned skins
            window.SpaceHubConfig.skins = cleanedAdminSkins;
            
            // Save the cleaned config back to JS Injector
            if (window.SpaceHubUtils && window.SpaceHubUtils.saveConfigToJavaScriptInjector) {
                const saveSuccess = await window.SpaceHubUtils.saveConfigToJavaScriptInjector(window.SpaceHubConfig, { waitForLogin: false });
                if (saveSuccess) {
                ...
                WARN('SpaceHubUtils.saveConfigToJavaScriptInjector not available, cannot save cleaned config');
            }
        ...
        window.SpaceHubSkinConfig = SKINS_CONFIG;
        
        const enabledCount = mergedSkins.filter(s => s.enabled !== false).length;
        const disabledCount = mergedSkins.length - enabledCount;
        LOG(`Loaded ${SKINS_CONFIG.length} enabled skins (${enabledCount} enabled, ${disabledCount} disabled). Merged config available at window.SpaceHubSkinConfig`);
    }
    ...
    function getSkinSourceInfo(skinName) {
        ...
        const defaultSkins = window.SpaceHubDefaultSkinsConfig?.skins || [];
        const adminSkins = window.SpaceHubConfig?.skins || [];
    ...
    function getAllSkinSources() {
        const sourceMap = {};
        const mergedSkins = window.SpaceHubSkinConfig || SKINS_CONFIG || [];
    ...
    window.SpaceHubSkinManager = window.SpaceHubSkinManager || {};
    window.SpaceHubSkinManager.getSkinSourceInfo = getSkinSourceInfo;
    window.SpaceHubSkinManager.getAllSkinSources = getAllSkinSources;
    
    // Load and merge theme configurations
    async function loadThemeConfig() {
        // Load themes from admin config only (no default themes support for now)
        const adminThemes = window.SpaceHubConfig?.themes || [];
    ...
    const USER_CONFIG_STORAGE_KEY = 'SpaceHubUserConfig';
     ...
     * 2. Admin config (SpaceHubConfig.optionalIncludes) - objects with { key, enabled }
    ...
    function isOptionalIncludeEnabled(key, defaultEnabled = false) {
        ...
        const adminOptionalIncludes = window.SpaceHubConfig?.optionalIncludes || [];
    ...
    function getMergedGlobalOptionalIncludes() {
        ...
        return (window.SpaceHubDefaultSkinsConfig?.globalOptionalIncludes || []).map(include => ({ ...include }));
    }
    ...
    function initialize(retryCount = 0) {
        ...
        if (!window.SpaceHubUtils) {
            ERR('SpaceHubUtils not available - skin manager cannot initialize');
            return;
        }
    ...
    function registerHandlers() {
        ...
        unregisterDisplayPreferencesHandler = window.SpaceHubUtils.onViewPage(handleDisplayPreferencesPage, {
            pages: ['mypreferencesdisplay']
        });
        
        // Register for all pages to add the appearance button
        unregisterAnyPageHandler = window.SpaceHubUtils.onViewPage(handleAnyPage, {
            pages: []
        });
    ...
    function getDefaultSkinName() {
        ...
        const configDefaultSkin = window.SpaceHubConfig?.defaultSkin;
