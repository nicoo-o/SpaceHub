...
(function() {
    ...
    const LOG = (...args) => console.log('[SpaceHub CustomMenuLinks]', ...args);
    const WARN = (...args) => console.warn('[SpaceHub CustomMenuLinks]', ...args);
    const ERR = (...args) => console.error('[SpaceHub CustomMenuLinks]', ...args);
    
    LOG('Script loaded - JS Injector mode');
    
    // Get custom menu links configuration
    function getCustomMenuLinksConfig() {
        return window.SpaceHubConfig?.customMenuLinks || [];
    }
    ...
    async function initializeCustomMenuLinks() {
        ...
        if (!window.SpaceHubUtils || !window.SpaceHubUtils.addCustomMenuLink) {
            ERR('SpaceHubUtils.addCustomMenuLink not available');
            return;
        }
    ...
    function waitForUtilsAndInitialize() {
        ...
        LOG('Waiting for SpaceHubUtils to be available');
        ...
        setTimeout(() => {
            clearInterval(checkInterval);
            WARN('SpaceHubUtils not available after 10 seconds');
        }, 10000);
    }
