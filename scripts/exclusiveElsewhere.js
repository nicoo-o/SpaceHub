...
(function() {
    ...
    const LOG = (...args) => console.log('[SpaceHub ExclusiveElsewhere]', ...args);
    const WARN = (...args) => console.warn('[SpaceHub ExclusiveElsewhere]', ...args);
    const ERR = (...args) => console.error('[SpaceHub ExclusiveElsewhere]', ...args);
    
    LOG('Script loaded - JS Injector mode');
    
    // Get configuration from global config
    const config = window.SpaceHubConfig?.exclusiveElsewhere || {
