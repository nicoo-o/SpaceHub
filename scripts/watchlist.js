...
(function() {
    ...
    const LOG = (...args) => console.log('[SpaceHub Watchlist]', ...args);
    const WARN = (...args) => console.warn('[SpaceHub Watchlist]', ...args);
    const ERR = (...args) => console.error('[SpaceHub Watchlist]', ...args);
	...
	function getCurrentSortOrder() {
		...
		const savedSort = localStorage.getItem('SpaceHub_progressSort');
	...
	function getCurrentSortDirection() {
		...
		// Fallback to localStorage
		const savedDirection = localStorage.getItem('SpaceHub_progressSortDirection');
	...
	function getCurrentMovieSortOrder() {
		...
		// Fallback to localStorage
		const savedSort = localStorage.getItem('SpaceHub_movieSort');
	...
	function getCurrentMovieSortDirection() {
		...
		// Fallback to localStorage
		const savedDirection = localStorage.getItem('SpaceHub_movieSortDirection');
		if (savedDirection && (savedDirection === 'asc' || savedDirection === 'desc')) {
			return savedDirection;
		}
	...
	function setSortOrder(sortKey, direction = null) {
		...
		localStorage.setItem('SpaceHub_progressSort', sortKey);
		localStorage.setItem('SpaceHub_progressSortDirection', sortDirection);
	...
	function setMovieSortOrder(sortKey, direction = null) {
		...
		// Save to localStorage
		localStorage.setItem('SpaceHub_movieSort', sortKey);
		localStorage.setItem('SpaceHub_movieSortDirection', sortDirection);
	...
	function showPlaylistSyncToast(message) {
		if (window.SpaceHubToaster && typeof window.SpaceHubToaster.toast === 'function') {
			window.SpaceHubToaster.toast(message);
		} else {
			LOG(message);
		}
	}
	...
	if (window.SpaceHubUtils && window.SpaceHubUtils.onViewPage) {
		...
		WARN('SpaceHubUtils.onViewPage not available');
	}
			...
			.then((data) => {
				...
				data.forEach((tab, index) => {
					if (tab.ContentHtml.indexOf('sections watchlist') !== -1) {
						...
						window.SpaceHubUtils.addCustomMenuLink(
							'Watchlist', 
							'bookmark', 
	...
	function initializeItemDetailPageHandler() {
		if (!window.SpaceHubUtils || !window.SpaceHubUtils.onViewPage) {
			WARN('SpaceHubUtils.onViewPage not available, retrying in 1 second');
			setTimeout(initializeItemDetailPageHandler, 1000);
			return;
		}

		LOG('Registering item detail page handler with SpaceHubUtils');

		window.SpaceHubUtils.onViewPage(async (view, element, hash, itemPromise) => {
		...
		LOG('Item detail page handler initialized via SpaceHubUtils');
	}
