// SpaceHub localStorage Cache Manager
...
(function() {
    ...
    const LOG = (...args) => console.log('[SpaceHub LocalStorageCache]', ...args);
    const WARN = (...args) => console.warn('[SpaceHub LocalStorageCache]', ...args);
    const ERR = (...args) => console.error('[SpaceHub LocalStorageCache]', ...args);
    
    // Generic localStorage cache manager with chunked storage support
    class LocalStorageCache {
        constructor(prefix = 'SpaceHub_', ttl = 24 * 60 * 60 * 1000, chunkSize = 10) { // 24 hours, 10 items per chunk
            this.prefix = prefix;
            this.ttl = ttl;
