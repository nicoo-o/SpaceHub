...
(function() {
    ...
    const LOG = (...args) => console.log('[SpaceHub HomeScreen]', ...args);
    const WARN = (...args) => console.warn('[SpaceHub HomeScreen]', ...args);
    const ERR = (...args) => console.error('[SpaceHub HomeScreen]', ...args);
    ...
    function manageBodyClasses() {
        ...
        const currentView = window.SpaceHubUtils?.getCurrentView();
        const isHomePage = currentView === 'home' || currentView === 'home.html';
        
        // Get seasonal animation setting
        const homeScreenConfig = window.SpaceHubConfig?.homeScreen || {};
            ...
            if (isHalloweenPeriod()) {
                ...
                if (enableSeasonalAnimations) {
                    ...
                    if (!snowverlayScript) {
                        ...
                        const snowverlayScriptSrc = (window.SpaceHubConfig.SpaceHubRoot || '') + 'scripts/snowverlay.js';
    ...
    if (window.SpaceHubUtils) {
        window.SpaceHubUtils.onViewPage(manageBodyClasses, {
            pages: []
        });
        LOG('Registered body class management for home page');
    } else {
        WARN('SpaceHubUtils not available, body class management may not work correctly');
    }
    ...
    async function migrateHomeScreenConfig() {
        if (!window.SpaceHubConfig || !window.SpaceHubConfig.homeScreen) {
            return;
        }
        
        const old = window.SpaceHubConfig.homeScreen;
        const migrated = {};
        ...
        if (old.enableNewAndTrending !== undefined || old.enableNewMovies !== undefined) {
            ...
            if (!window.SpaceHubConfig.homeScreen.upcoming) {
                migrated.upcoming = {
                    enabled: true,
                    itemLimit: 48,
                    cardFormat: "Thumb",
            ...
            if (!window.SpaceHubConfig.homeScreen.imdbTop250) {
                migrated.imdbTop250 = {
                    enabled: true,
                    itemLimit: 16,
                    sortOrder: "Random",
            ...
            if (!window.SpaceHubConfig.homeScreen.watchAgain) {
                migrated.watchAgain = {
                    enabled: false,
                    itemLimit: 16,
                    sortOrder: "Random",
            ...
            window.SpaceHubConfig.homeScreen = {
                ...migrated,
                ...window.SpaceHubConfig.homeScreen
            };
            ...
            if (window.SpaceHubUtils && window.SpaceHubUtils.saveConfigToJavaScriptInjector) {
                window.SpaceHubUtils.saveConfigToJavaScriptInjector().catch(err => {
                    ERR('Failed to save migrated config to JS Injector:', err);
                });
            } else {
                WARN('SpaceHubUtils.saveConfigToJavaScriptInjector not available, config migration not persisted');
            }
        }
        ...
        const currentConfig = window.SpaceHubConfig.homeScreen;
        ...
        if (configChanged && !needsMigration) {
            ...
            if (window.SpaceHubUtils && window.SpaceHubUtils.saveConfigToJavaScriptInjector) {
                await window.SpaceHubUtils.saveConfigToJavaScriptInjector();
            }
        }
    }
...
/*             if (!sectionConfig.source && (sectionConfig.sourceId || sectionConfig.id)) {
    ...
    async function renderAllRecentlyAddedInLibrarySections(container) {
        ...
        const config = window.SpaceHubConfig?.homeScreen?.recentlyAddedInLibrary || {};
    ...
    async function getWatchlistUrl() {
        ...
        if (!window.SpaceHubUtils || !window.SpaceHubUtils.getWatchlistTabIndex || !window.SpaceHubUtils.getWatchlistTabIndex()) {
            WARN('SpaceHubUtils not available');
            return null;
        }

        const visibleWatchlistTabIndex = await window.SpaceHubUtils.getWatchlistTabIndex();
    ...
    function cacheImdbIds(imdbIds) {
        try {
            localStorage.setItem('SpaceHub_imdbIds', JSON.stringify({ 
                ids: imdbIds, 
                timestamp: Date.now() 
            }));
        } catch (err) {
    ...
    function getCachedImdbIds() {
        try {
            const cached = localStorage.getItem('SpaceHub_imdbIds');
    ...
    if (window.SpaceHubUtils) {
        LOG('Registering home screen handler with SpaceHubUtils');
        
        // Register handler for all pages (breadcrumbs can appear on any detail page)
        window.SpaceHubUtils.onViewPage((view, element) => {
            try {
                // Run our custom code
        ...
        ERR('SpaceHubUtils not available, breadcrumbs may not work correctly');
    }
