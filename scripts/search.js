...
(function() {
    'use strict';
    const LOG = (...args) => console.log('[SpaceHub Search]', ...args);
    const WARN = (...args) => console.warn('[SpaceHub Search]', ...args);
    const ERR = (...args) => console.error('[SpaceHub Search]', ...args);
    
    LOG('Initializing...');

    // Configuration
    const CONFIG = window.SpaceHubConfig?.search || {
    ...
    function initializeSearchHook() {
        ...
        LOG('Registering search page handler with SpaceHubUtils');
