...
(function() {
    ...
    const LOG = (...args) => console.log('[SpaceHub SubtitleSearch]', ...args);
    const WARN = (...args) => console.warn('[SpaceHub SubtitleSearch]', ...args);
    const ERR = (...args) => console.error('[SpaceHub SubtitleSearch]', ...args);
    ...
    function showDownloadProgress() {
        if (window.SpaceHubToaster && window.SpaceHubToaster.toast) {
            window.SpaceHubToaster.toast('Downloading subtitle...', null, false);
        }
    }
    ...
    function showDownloadSuccess() {
        ...
        if (window.SpaceHubToaster && window.SpaceHubToaster.toast) {
            window.SpaceHubToaster.toast('Subtitle downloaded and activated!');
        }
    }
    ...
    function showDownloadError(message) {
        ...
        if (window.SpaceHubToaster && window.SpaceHubToaster.toast) {
            window.SpaceHubToaster.toast(message || 'Failed to download subtitle', '5');
        }
    }
    ...
    async function initialize() {
        ...
        try {
            ...
            // Wait for SpaceHubUtils to be available
            if (!window.SpaceHubUtils || !window.SpaceHubUtils.onViewPage) {
                LOG('SpaceHubUtils.onViewPage not available, retrying in 1 second');
                setTimeout(initialize, 1000);
                return;
            }
            
            // Register onViewPage handler for video page
            onViewPageUnregister = window.SpaceHubUtils.onViewPage(async (view, element, hash) => {
