...
export class UnifiedCalendarWidget {
    ...
    async _loadEvents(container) {
        ...
        try {
            ...
            if (events.length === 0) {
                contentEl.innerHTML = `
                    <div style="padding: 28px 20px; text-align: center; color: rgba(255,255,255,0.7); background: rgba(255,255,255,0.03); border: 1px dashed rgba(255,255,255,0.12); border-radius: 16px; display: flex; flex-direction: column; align-items: center; gap: 8px;">
                        <span style="font-size: 28px;">📅</span>
                        <strong style="color: #ffffff; font-size: 15px;">Calendrier des Sorties Synchronisé</strong>
                        <p style="margin: 0; font-size: 13px; color: rgba(255,255,255,0.5); max-width: 480px;">Renseignez vos clés API Sonarr, Radarr ou Jellyseerr dans l'onglet <strong>Intégrations</strong> des réglages pour afficher automatiquement la chronologie des sorties quotidiennes.</p>
                    </div>
                `;
                return;
            }
