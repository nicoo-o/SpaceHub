// SpaceHub Script Injector
// Dynamically loads scripts and CSS files based on user configuration
// Usage: Configuration is now managed in SpaceHub.js - this script reads from window.SpaceHubConfig

(function () {
    'use strict';

    console.log('[SpaceHub Injector] Initializing...');

    // Check for global configuration reset
    try {
        const config = window.SpaceHubConfig || {};
        const remoteVersion = config.configVersion || 0;
        const localVersion = parseInt(localStorage.getItem('SpaceHub_lastResetVersion') || '0', 10);

        if (remoteVersion > localVersion && remoteVersion > 0) {
            console.log('[SpaceHub] Global reset detected. Clearing user configuration.');
            // Only clear the user config to force defaults
            // Remove all keys starting with 'SpaceHub'
            Object.keys(localStorage)
                .filter(key => key.startsWith('SpaceHub'))
                .forEach(key => localStorage.removeItem(key));
            // Update local version to avoid loop
            localStorage.setItem('SpaceHub_lastResetVersion', remoteVersion.toString());
        }
    } catch (e) {
        console.error('[SpaceHub] Error checking for global reset:', e);
    }
    ...
    const SCRIPT_DEFINITIONS = [
        ...
        },
        {
            ...
            css: null,
            dependencies: [],
            description: 'Default skin configuration for SpaceHub'
        },
        {
            name: 'skinManager',
    ...
    async function injectVersionBadgeCSS() {
        const styleId = 'SpaceHub-version-css';
        if (document.getElementById(styleId)) return;

        // Get version from root URL in config
        let displayVersion = "";
        try {
            const configRoot = window.SpaceHubConfig?.SpaceHubRoot || '';
            if (configRoot) {
            ...
            console.warn('[SpaceHub Injector] Could not extract version, using fallback:', error);
        }
        ...
        style.textContent = `
            body:not(:has(.libraryPage:not(.hide))) .MuiPaper-root.MuiDrawer-paperAnchorLeft::after {
                content: 'SpaceHub ${versionDisplay}';
                font-style: italic;
                text-align: center;
                margin-top: 10px;
                color: #b7b7b7;
    ...
    function autoEnableDependencies() {
                ...
                if (ENABLED_SCRIPTS[script.name]) {
                    script.dependencies.forEach(dep => {
                        if (!ENABLED_SCRIPTS[dep]) {
                            ...
                            hasChanges = true;
                            console.log(`[SpaceHub Injector] Auto-enabled dependency '${dep}' for '${script.name}'`);
                        }
                    });
                }
            });
        ...
        if (iterations >= maxIterations) {
            console.warn('[SpaceHub Injector] Reached max iterations while auto-enabling dependencies');
        }
    ...
    function validateConfiguration() {
        ...
        if (dependenciesEnabled) {
            console.log('[SpaceHub Injector] Auto-enabled required dependencies');
        }
        ...
        if (errors.length > 0) {
            console.error('[SpaceHub Injector] Configuration errors:', errors);
            return false;
        }
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (data && data.tag_name) {
                        ...
                        console.warn('[SpaceHub Injector] Could not fetch latest version, using @latest');
                        return root;
                    }
                } else {
                    console.warn('[SpaceHub Injector] Failed to fetch latest version, using @latest');
                    return root;
                }
            } catch (error) {
                console.warn('[SpaceHub Injector] Error fetching latest version:', error);
                return root;
            }
        } else if (mainMatch) {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (commit && commit.sha) {
                        ...
                        console.warn('[SpaceHub Injector] Could not fetch commit hash, using @main');
                        return root;
                    }
                } else {
                    console.warn('[SpaceHub Injector] Failed to fetch commit hash, using @main');
                    return root;
                }
            } catch (error) {
                console.warn('[SpaceHub Injector] Error fetching commit hash:', error);
                return root;
            }
        }
    ...
    async function getScriptRoot() {
        const SpaceHubRoot = window.SpaceHubConfig?.SpaceHubRoot;

        // If SpaceHubRoot is not set or empty, return null (scripts should not load)
        if (!SpaceHubRoot || SpaceHubRoot === '') {
            console.warn('[SpaceHub Injector] SpaceHubRoot is not configured');
            return null;
        }

        // Ensure root ends with /
        let root = SpaceHubRoot.endsWith('/') ? SpaceHubRoot : SpaceHubRoot + '/';
    ...
    async function loadCSS(filename) {
        return new Promise(async (resolve, reject) => {
            ...
            if (!scriptRoot) {
                reject(new Error('SpaceHubRoot is not configured'));
                return;
            }
            ...
            if (existingLink) {
                console.log(`[SpaceHub Injector] CSS already loaded: ${filename}`);
                resolve();
                return;
            }
            ...
            link.onload = () => {
                console.log(`[SpaceHub Injector] CSS loaded: ${filename}`);
                resolve();
            };

            link.onerror = () => {
                console.warn(`[SpaceHub Injector] Failed to load CSS: ${filename}`);
    ...
    async function loadScript(filename) {
        return new Promise(async (resolve, reject) => {
            ...
            if (existingScript) {
                console.log(`[SpaceHub Injector] Script already loaded: ${filename}`);
                resolve();
                return;
            }
            ...
            script.onload = () => {
                console.log(`[SpaceHub Injector] Script loaded: ${filename}`);
                resolve();
            };

            script.onerror = () => {
                console.error(`[SpaceHub Injector] Failed to load script: ${filename}`);
    ...
    async function loadScriptSync(scriptDef) {
        const scriptRoot = await getScriptRoot();
        if (!scriptRoot) {
            throw new Error('SpaceHubRoot is not configured');
        }
        ...
        if (isAlreadyLoaded) {
            console.log(`[SpaceHub Injector] Script already loaded: ${scriptDef.name}`);
            return;
        }
        ...
        console.log(`[SpaceHub Injector] Successfully loaded: ${scriptDef.name}`);
    }
        ...
        for (const depName of configDependencyNames) {
            ...
            if (depScript) {
                try {
                    ...
                    console.warn(`[SpaceHub Injector] Failed to load configuration dependency '${depName}':`, error);
                }
            }
        }

        // Load configuration UI so admins can re-enable SpaceHub
    ...
    async function initialize() {
        console.log(`[SpaceHub Injector] Starting SpaceHub initialization...`);

        const scriptRoot = await getScriptRoot();
        if (!scriptRoot) {
            console.log('[SpaceHub Injector] SpaceHubRoot is not configured. Please configure SpaceHub using the installer before scripts can be loaded.');
            return;
        }

        // Check if SpaceHub is enabled in config
        const isSpaceHubEnabled = window.SpaceHubConfig?.enabled !== false;

        if (!isSpaceHubEnabled) {
            console.log('[SpaceHub Injector] SpaceHub is disabled in configuration. Only loading configuration UI requirements for admin access.');

            try {
                // Ensure configuration dependencies are loaded so the admin UI works
                await loadConfigurationJS();
                console.log('[SpaceHub Injector] Configuration script loaded (SpaceHub is disabled)');
            } catch (error) {
                console.warn('[SpaceHub Injector] Failed to load configuration UI while disabled:', error);
            }
            return;
        }
        ...
        if (!validateConfiguration()) {
            console.error('[SpaceHub Injector] Configuration validation failed. Aborting.');
            return;
        }
        ...
        console.log(`[SpaceHub Injector] Found ${dependencyScripts.length} dependency scripts:`,
            dependencyScripts.map(s => s.name));
        console.log(`[SpaceHub Injector] Found ${priorityScripts.length} priority scripts:`,
            priorityScripts.map(s => s.name));
        console.log(`[SpaceHub Injector] Found ${regularScripts.length} regular scripts:`,
            regularScripts.map(s => s.name));

        try {
            // Step 3: Load all dependencies first
            console.log('[SpaceHub Injector] Loading dependencies first...');
            console.log(`[SpaceHub Injector] Dependency load order:`, dependencyScripts.map(s => s.name));
            ...
            if (priorityScripts.length > 0) {
                console.log('[SpaceHub Injector] Loading priority scripts...');
                console.log(`[SpaceHub Injector] Priority load order:`, priorityScripts.map(s => s.name));
            ...
            let loadPromises = [];
            console.log('[SpaceHub Injector] Loading regular non-dependencies...');
            loadPromises.push(...regularScripts.map(script => loadScriptSync(script)));
            await Promise.all(loadPromises);

            console.log('[SpaceHub Injector] All scripts loaded successfully!');

            // Always load configuration.js for admin UI (loads after other scripts)
            console.log('[SpaceHub Injector] Loading configuration script...');
            try {
                await loadConfigurationJS();
                console.log('[SpaceHub Injector] Configuration script loaded successfully');
            } catch (error) {
                console.warn('[SpaceHub Injector] Failed to load configuration script:', error);
            }

            // Dispatch custom event when all scripts are loaded
            const event = new CustomEvent('SpaceHubLoaded', {
                detail: {
            ...
            console.error('[SpaceHub Injector] Error during initialization:', error);
        }
    }
    ...
    window.SpaceHub = {
        ...
        reload: () => {
            console.log('[SpaceHub Injector] Reloading all scripts...');
            initialize();
        },
        ...
        return new Promise((resolve) => {
            const checkAdmin = async () => {
                try {
                    if (window.ApiClient && window.ApiClient.getCurrentUser) {
                        ...
                        if (user && user.Policy && user.Policy.IsAdministrator === true) {
                            console.log('[SpaceHub Startup] User is admin');
                            resolve(true);
                            return;
                        }
                        // User is logged in but not admin
                        if (user) {
                            console.log('[SpaceHub Startup] User is not admin');
                            resolve(false);
                            return;
                        }
                    }
                ...
                if (Date.now() - startTime >= maxWaitMs) {
                    console.log('[SpaceHub Startup] Timeout waiting for user login');
                    resolve(false);
                    return;
                }
    ...
    async function findPlugin(pluginName) {
        try {
            ...
            console.error(`[SpaceHub Startup] Error finding plugin ${pluginName}:`, error);
            return null;
        }
    }
    ...
    async function getPluginConfig(pluginId) {
        try {
            ...
            console.error('[SpaceHub Startup] Error getting plugin config:', error);
            throw error;
        }
    }
    ...
    async function savePluginConfig(pluginId, config) {
        try {
            ...
            console.error('[SpaceHub Startup] Error saving plugin config:', error);
            throw error;
        }
    }
    ...
    async function ensureWatchlistTab() {
        try {
            if (!window.SpaceHubConfig) {
                console.log('[SpaceHub Startup] SpaceHubConfig not found, skipping Watchlist tab check');
                return true;
            }

            // Check if Watchlist is enabled in the configuration and remove it if not
            if (!window.SpaceHubConfig.scripts.watchlist) {
                console.log('[SpaceHub Startup] Watchlist is disabled in configuration, removing Watchlist tab');
                const pluginId = await findPlugin('Custom Tabs');
                if (!pluginId) {
                    console.warn('[SpaceHub Startup] CustomTabs plugin not found');
                    return false;
                }
                ...
                console.log('[SpaceHub Startup] Removed Watchlist tab from CustomTabs');
                return true;
            }

            console.log('[SpaceHub Startup] Ensuring Watchlist tab exists in CustomTabs...');
            ...
            if (watchlistTab) {
                console.log('[SpaceHub Startup] Watchlist tab already exists with correct ContentHtml');
                return true;
            }
            ...
            // Save updated config
            await savePluginConfig(pluginId, { Tabs: tabs });
            console.log('[SpaceHub Startup] Added Watchlist tab to CustomTabs');
            return true;
        } catch (error) {
            console.error('[SpaceHub Startup] Error ensuring Watchlist tab:', error);
            return false;
        }
    }
    ...
    async function startupTask() {
        console.log('[SpaceHub Startup] Starting startup task...');

        // Check if SpaceHubRoot is configured - startup tasks only run after installation
        if (!window.SpaceHubConfig?.SpaceHubRoot || window.SpaceHubConfig.SpaceHubRoot === '') {
            console.log('[SpaceHub Startup] SpaceHubRoot is not configured, skipping startup tasks');
            return;
        }
        ...
        if (!isAdmin) {
            console.log('[SpaceHub Startup] User is not admin or not logged in, skipping startup task');
            return;
        }
        ...
        try {
            ...
            console.log('[SpaceHub Startup] Startup task completed successfully');
        } catch (error) {
            console.error('[SpaceHub Startup] Error in startup task:', error);
        }
    }
    ...
    console.log('[SpaceHub Injector] Injector script loaded. Available at window.SpaceHub');
