    ...
    constructor() {
        ...
        if (this._eventBus) {
            this._eventBus.on('settings:changed', ({ key, value }) => {
                ...
                if (key === 'prowlarr.url' || (key === '*' && value['prowlarr.url'])) {
                    this.api.setBaseUrl(key === '*' ? value['prowlarr.url'] : value);
                }
                if (key === 'prowlarr.apiKey' || (key === '*' && value['prowlarr.apiKey'])) {
                    this.api.setApiKey(key === '*' ? value['prowlarr.apiKey'] : value);
                }
            });
        }
    }
