...
function renderJellyseerrMediaCard(item) {
    ...
    return `
        ...
        <div tabindex="0" data-nav-focusable="true" data-nav-scope="jellyseerr" class="sh-jellyseerr-bento-card" data-media-id="${escapeHtml(String(item.id ?? ''))}" data-media-type="${type}" data-overview="${escapeHtml(encodeURIComponent(item.overview || ''))}">
            ...
            contentEl.innerHTML = `
                <div style="display:flex; flex-direction:column; gap:10px;">
                    ${requests.map(req => {
                        ...
                        return `
                            <div class="sh-jellyseerr-request-card" style="display: flex; align-items: center; gap: 14px; background: rgba(var(--sh-ink, 255, 255, 255),  0.03); border: 1px solid rgba(var(--sh-ink, 255, 255, 255),  0.07); border-radius: 14px; padding: 10px 14px;">
                                <div class="sh-jellyseerr-request-card__poster" style="width: 44px; height: 66px; flex-shrink: 0; border-radius: 8px; overflow: hidden; background: rgba(var(--sh-ink-inv, 0, 0, 0.16), 0.3);">
                                    ${poster ? `<img src="${poster}" alt="${escapeHtml(title)}" style="width:100%; height:100%; object-fit:cover;" loading="lazy"/>` : '<div class="sh-placeholder" style="width:100%; height:100%; display:flex; align-items:center; justify-content:center; background:rgba(255,255,255,0.18);"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect width="20" height="20" x="2" y="2" rx="2.18"></rect><line x1="7" y1="2" x2="7" y2="22"></line><line x1="17" y1="2" x2="17" y2="22"></line><line x1="2" y1="12" x2="22" y2="12"></line></svg></div>'}
                                </div>
                                <div class="sh-jellyseerr-request-card__details" style="flex:1;">
                                    <h4 class="sh-jellyseerr-request-card__title sh-truncate" style="margin:0 0 4px 0; color:var(--sh-ink-solid, #ffffff); font-size:14px;">${escapeHtml(title)}</h4>
                                    <p class="sh-jellyseerr-request-card__user" style="margin:0; font-size:11.5px; color:rgba(var(--sh-ink, 255, 255, 255), 0.5);">Demandé par <strong style="color:rgba(var(--sh-ink, 255, 255, 255), 0.85);">${escapeHtml(user.displayName || user.email || 'Utilisateur')}</strong></p>
                                </div>
