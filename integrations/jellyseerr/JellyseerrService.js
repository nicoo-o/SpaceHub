    ...
    constructor() {
        ...
        if (this._eventBus) {
            this._eventBus.on('settings:changed', ({ key, value }) => {
                ...
                if (key === 'jellyseerr.url' || (key === '*' && value['jellyseerr.url'])) {
                    this.api.setBaseUrl(key === '*' ? value['jellyseerr.url'] : value);
                }
                if (key === 'jellyseerr.apiKey' || (key === '*' && value['jellyseerr.apiKey'])) {
                    this.api.setApiKey(key === '*' ? value['jellyseerr.apiKey'] : value);
                }
            });
        }
    }
