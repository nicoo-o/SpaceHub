...
class QBittorrentApi {
    ...
    /**
     * Définit l'URL de base temporairement (pour test sans sauvegarde)
     * @param {string} url
     */
    setBaseUrl(url) {
    ...
    /**
     * Définit le nom d'utilisateur temporairement (pour test sans sauvegarde)
    ...
    /**
     * Définit le mot de passe temporairement (pour test sans sauvegarde)
    ...
    /**
     * Résout l'URL réelle vers l'URL de dev-proxy (contourne le CORS de
     * qBittorrent, qui n'envoie pas d'en-tête Access-Control-Allow-Origin).
     * Voir vite.config.js — sans effet en production.
     */
    _resolveUrl(path) {
        const realUrl = `${this.baseUrl}${path}`;
        const isDev = typeof import.meta !== 'undefined' && import.meta.env?.DEV;
        return isDev ? `/__sh-proxy?target=${encodeURIComponent(realUrl)}` : realUrl;
    }
    ...
    async login() {
        this.updateConfig();
        const url = this._resolveUrl('/api/v2/auth/login');
